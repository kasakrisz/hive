 ...
 */
class TestGenericUDAFNoNullLag {

  GenericUDAFLag.GenericUDAFLagEvaluatorStreaming evaluator;

  @BeforeEach
  void setUp() throws HiveException {
    GenericUDAFLag.GenericUDAFNoNullLagEvaluator baseEvaluator = new GenericUDAFLag.GenericUDAFNoNullLagEvaluator();
    ...
    evaluator = new GenericUDAFLag.GenericUDAFLagEvaluatorStreaming(baseEvaluator);
  }
  ...
  private GenericUDAFLag.NoNullLagBuffer leadBuffer() {
    GenericUDAFLag.NoNullLagBuffer buffer = new GenericUDAFLag.NoNullLagBuffer();
    buffer.initialize(2);
    return buffer;
  }
  ...
  void testIgnoreNulls() throws HiveException {
    ...
    assertThat(evaluator.getNextResult(buffer), is(new IntWritable(8)));

    evaluator.iterate(buffer, parameters(null));
    assertThat(evaluator.getNextResult(buffer), is(new IntWritable(5)));

    evaluator.iterate(buffer, parameters(2));
    assertThat(evaluator.getNextResult(buffer), is(new IntWritable(5)));

    evaluator.iterate(buffer, parameters(1));
