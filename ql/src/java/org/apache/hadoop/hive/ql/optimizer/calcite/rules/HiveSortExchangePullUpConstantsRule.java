...
public class HiveSortExchangePullUpConstantsRule extends RelOptRule {
  ...
  public static final HiveSortExchangePullUpConstantsRule INSTANCE =
          new HiveSortExchangePullUpConstantsRule(HiveSortExchange.class,
                  HiveRelFactories.HIVE_BUILDER);

  private HiveSortExchangePullUpConstantsRule(Class<? extends SortExchange> sortClass,
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    final SortExchange sortExchange = call.rel(1);

    final int count = sortExchange.getInput().getRowType().getFieldCount();
    if (count == 1) {
    ...
    final RexBuilder rexBuilder = sortExchange.getCluster().getRexBuilder();
    final RelMetadataQuery mq = call.getMetadataQuery();
    final RelOptPredicateList predicates = mq.getPulledUpPredicates(sortExchange.getInput());
    if (predicates == null) {
      return;
    }
    ...
    for (int i = 0; i < count ; i++) {
      RexNode expr = rexBuilder.makeInputRef(sortExchange.getInput(), i);
    ...
    List<RelDataTypeField> fields = sortExchange.getInput().getRowType().getFieldList();
    ...
    final Mappings.TargetMapping mapping =
            RelOptUtil.permutation(Pair.left(newChildExprs), sortExchange.getInput().getRowType()).inverse();
    ...
    relBuilder.push(sortExchange.getInput());
...
//            sortExchange.fetch == null ? -1 : RexLiteral.intValue(sortExchange.fetch), sortFields);
    ...
    relBuilder.convert(sortExchange.getRowType(), false);

    List<RelNode> inputs = new ArrayList<>();
    for (RelNode child : parent.getInputs()) {
      if (!((HepRelVertex) child).getCurrentRel().equals(sortExchange)) {
        inputs.add(child);
      } else {
