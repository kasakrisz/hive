...
public class MergeSemanticAnalyzer extends RewriteSemanticAnalyzer {
  ...
  public void analyze(ASTNode tree, Table targetTable, ASTNode tableNameNode) throws SemanticException {
    ...
    if (nonNativeAcid) {
      throw new SemanticException(ErrorMsg.NON_NATIVE_ACID_MERGE.getErrorCodedMsg());
    }
  ...
  protected void analyzeMerge(ASTNode tree, Table targetTable, ASTNode targetNameNode)
    ...
    */
    ...
    ColumnAppender columnAppender = createColumnAppender(targetTable, subQueryAlias);
    ...
    for (int whenClauseIdx = 0;
      ...
      case HiveParser.TOK_INSERT:
        ...
        break;
      case HiveParser.TOK_UPDATE:
        for (Context.DestClausePrefix destClausePrefix : getUpdateDestNamePrefix()) {
  ...
  protected Context.DestClausePrefix[] getUpdateDestNamePrefix() {
