...
public class ASTBuilder {
  ...
  public static ASTNode table(final RelNode scan) {
    ...
    if (hTbl.getHiveTableMD().getFromTimestamp() != null) {
      ASTBuilder fromBuilder = ASTBuilder.construct(HiveParser.TOK_START_TIME, "TOK_FROM_TIME")
          .add(HiveParser.StringLiteral, hTbl.getHiveTableMD().getFromTimestamp());
      b.add(fromBuilder);
      if (hTbl.getHiveTableMD().getToTimestamp() != null) {
        ASTBuilder toBuilder = ASTBuilder.construct(HiveParser.TOK_END_TIME, "TOK_TO_TIME")
    ...
    if (hTbl.getHiveTableMD().getFromVersion() != null) {
      ASTBuilder fromBuilder = ASTBuilder.construct(HiveParser.TOK_START_VERSION, "TOK_FROM_VERSION")
          .add(HiveParser.StringLiteral, hTbl.getHiveTableMD().getFromVersion());
      b.add(fromBuilder);
      if (hTbl.getHiveTableMD().getToVersion() != null) {
        ASTBuilder toBuilder = ASTBuilder.construct(HiveParser.TOK_END_VERSION, "TOK_TO_VERSION")
